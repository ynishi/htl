...
That covers the test files themselves. Modules a test `require`s at run time are still
generated by the runtime searcher on each run, so the saving is smaller than the checking it
removes: measured on a 27-file suite, 4.65 s to 3.6 s.
