...
fn diagnostics_carry_fixes_with_applicability() {
    ...
    write(&dir.join("num.tl"), "local total = 0\ntotal = total + 1.5\nprint(total)\n");
    write(&dir.join("glob.tl"), "global counter: integer = 0\nprint(counter)\n");
    ...
    assert_eq!(fixes.len(), 2, "both forward references get a fix: {:?}", w.errors);
    assert!(fixes.iter().all(|f| f.applicability == Applicability::Safe));
    let e = &fixes[0].edits[0];
    assert_eq!((e.line, e.col, e.end_line, e.end_col), (6, 1, 6, 1), "inserted before the record's `end`: {e:?}");
    assert_eq!(e.text, "   observe: function(w: world.W, what: string)\n", "indented like the last field");
    assert_eq!(fixes[1].edits[0].text, "   alive: function(self: world, w: world.W): boolean\n");

    let n = h.check(&dir.join("num.tl")).unwrap();
    let f = n.lint_fixes[0].as_ref().expect("explicit-number carries a fix");
    assert_eq!(f.applicability, Applicability::Safe);
    assert_eq!((f.edits[0].line, f.edits[0].col, &f.edits[0].text[..]), (1, 12, ": number"));
    ...
    assert_eq!((&f.edits[0].text[..], f.edits[0].col, f.edits[0].end_col), ("local", 1, 7));
}
...
fn fix_file_applies_safe_fixes_and_is_idempotent() {
    ...
    assert!(out.applied.iter().all(|a| a.rule == "forward-ref" && a.pass == 1));
    ...
    assert!(again.applied.is_empty() && again.contents.is_none(), "second run is a no-op");
}
...
fn unsafe_fixes_need_the_flag_or_promotion() {
    ...
    assert!(out.skipped.iter().any(|s| s.rule == "no-global" && s.reason.contains("--unsafe")), "{:?}", out.skipped);

    let out = fix_file(&h, &dir.join("glob.tl"), &FixOptions { unsafe_fixes: true, ..Default::default() }).unwrap();
    assert_eq!(out.applied.len(), 1);
    assert!(std::fs::read_to_string(dir.join("glob.tl")).unwrap().starts_with("local counter"));

    write(&dir.join("glob2.tl"), "global counter: integer = 0\nprint(counter)\n");
    let out = fix_file(&h, &dir.join("glob2.tl"), &FixOptions { promoted: vec!["no-global".into()], ..Default::default() }).unwrap();
...
fn dry_run_and_diff_leave_the_file_alone() {
    ...
    h.configure_lints("+explicit-number").unwrap();
    h.add_path(&dir).unwrap();
    let out = fix_file(&h, &dir.join("num.tl"), &FixOptions { dry_run: true, ..Default::default() }).unwrap();
    assert_eq!(out.applied.len(), 1);
    let after = out.contents.as_deref().expect("what would be written");
    assert!(after.starts_with("local total: number = 0\n"), "{after}");
    assert_eq!(std::fs::read_to_string(dir.join("num.tl")).unwrap(), "local total = 0\ntotal = total + 1.5\nprint(total)\n", "untouched");
    let d = unified_diff("num.tl", "local total = 0\ntotal = total + 1.5\nprint(total)\n", after);
    assert!(d.contains("-local total = 0") && d.contains("+local total: number = 0"), "{d}");
}
...
fn other_type_errors_do_not_block_but_syntax_errors_do() {
    ...
    assert_eq!(out.check.errors.len(), 1, "the string error stays, the number one is gone: {:?}", out.check.errors);

    write(&dir.join("broken.tl"), "local total = 0\ntotal = total + 1.5\nif total then\n");
    let out = fix_file(&h, &dir.join("broken.tl"), &FixOptions::default()).unwrap();
    assert!(out.applied.is_empty() && out.contents.is_none());
    assert!(out.skipped.iter().any(|s| s.reason.contains("syntax error")), "{:?}", out.skipped);
}
...
fn a_fix_that_makes_things_worse_is_reverted() {
    ...
    write(&dir.join("world.tl"), &FWD.replacen("   name: string\nend\n", "end\n", 1));
    let out = fix_file(&h, &dir.join("world.tl"), &FixOptions::default()).unwrap();
    ...
    assert!(c.ok() || out.reverted.is_some(), "never leave a worse file behind: {:?} / {:?}\n{text}", c.errors, out.reverted);
}
